...
16. Niemals `Co-Authored-By`-Trailer in Commit-Nachrichten einfügen. Commits müssen ausschließlich unter der Git-Identität des Repository-Besitzers erscheinen (`diegosouzapw`). Die Zeile `Co-Authored-By: Claude …` führt dazu, dass GitHub Commits dem `claude` Anthropic-Konto zuordnet, wodurch der tatsächliche Autor in der PR-Historie verborgen bleibt.
