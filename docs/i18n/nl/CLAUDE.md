...
16. Nooit `Co-Authored-By` trailers opnemen in commitberichten. Commits moeten uitsluitend onder de Git-identiteit van de repository-eigenaar verschijnen (`diegosouzapw`). De regel `Co-Authored-By: Claude …` zorgt ervoor dat GitHub commits toeschrijft aan het `claude` Anthropic-account, waardoor de echte auteur in de PR-geschiedenis verborgen blijft.
