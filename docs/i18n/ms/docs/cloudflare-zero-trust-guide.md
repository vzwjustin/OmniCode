# Guia Completo: Cloudflare Tunnel & Zero Trust (Split-Port) (Bahasa Melayu)
...
Este guia documenta o padrão ouro de infraestrutura de rede para proteger o **OmniCode** e expor sua aplicação de forma segura para a internet, **sem abrir nenhuma porta (Zero Inbound)**.

## O que foi feito na sua VM?

Nós ativamos o OmniCode em modo **Split-Port** através do PM2:
...
3. Clique em **Add a Tunnel**, escolha **Cloudflared** e dê o nome \`OmniCode-VM\`.
...
3. Em **Application name**, coloque \`Painel OmniCode\`.
...
4. **Name:** \`Anti-Abuso OmniCode API\`
...
2. O OmniCode só conversa HTTPS saindo (\`cloudflared\`) e não recebendo TCP direto do mundo.
