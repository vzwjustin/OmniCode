...
16. هرگز `Co-Authored-By` را در پیام‌های کامیت شامل نکنید. کامیت‌ها باید فقط تحت هویت Git مالک مخزن (`diegosouzapw`) ظاهر شوند. خط `Co-Authored-By: Claude …` باعث می‌شود GitHub کامیت‌ها را به حساب `claude` Anthropic نسبت دهد و نویسنده واقعی را در تاریخ PR پنهان کند.
