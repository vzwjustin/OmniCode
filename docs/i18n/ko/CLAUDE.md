...
16. 커밋 메시지에 `Co-Authored-By` 트레일러를 포함하지 마세요. 커밋은 반드시 저장소 소유자의 Git 신원 (`diegosouzapw`) 아래에만 나타나야 합니다. `Co-Authored-By: Claude …` 줄은 GitHub가 커밋을 `claude` Anthropic 계정에 귀속시켜 PR 기록에서 실제 저자를 숨깁니다.
