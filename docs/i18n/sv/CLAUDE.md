...
16. Inkludera aldrig `Co-Authored-By` trailers i commit-meddelanden. Commits måste endast visas under repository-ägarens Git-identitet (`diegosouzapw`). Raden `Co-Authored-By: Claude …` gör att GitHub attribuerar commits till `claude` Anthropic-kontot, vilket döljer den verkliga författaren i PR-historiken.
