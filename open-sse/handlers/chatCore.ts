...
import { buildOmniCoderResponseMetaHeaders } from "@/domain/omnirouteResponseMeta";
...
export async function handleChatCore({
  ...
  if (webSearchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webSearchFallbackPlan.convertedToolCount} web_search tool(s) to OmniCoder fallback for ${provider}`
    );
  }
