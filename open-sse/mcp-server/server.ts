...
async function omniRouteFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`OmniCoder API error [${response.status}]: ${errorText}`);
  }
...
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Returns OmniCoder health status including uptime, memory, circuit breakers, rate limits, and cache stats",
  ...
  server.registerTool(
    "omniroute_route_request",
    {
      description: "Sends a chat completion request through OmniCoder intelligent routing",
  ...
  server.registerTool(
    ...
    {
      description:
        "Diagnoses or repairs OmniCoder database drift, including broken combo references and orphan quota/domain rows",
  ...
  server.registerTool(
    ...
    {
      description:
        "Syncs pricing data from external sources (LiteLLM) into OmniCoder without overwriting user-set prices",
  ...
  server.registerTool(
    ...
    {
      description:
        "Performs a web search using OmniCoder's search gateway. Supports multiple providers (Serper, Brave, Perplexity, Exa, Tavily) with automatic failover. Returns search results with titles, URLs, snippets, and position data.",
...
export async function startMcpStdio(): Promise<void> {
  ...
  console.error("[MCP] OmniCoder MCP Server starting (stdio transport)...");
  try {
    await server.connect(transport);
    console.error("[MCP] OmniCoder MCP Server connected and ready.");
  } finally {
    if (closeAuditDb()) {
