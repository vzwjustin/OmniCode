...
export class GeminiCLIExecutor extends BaseExecutor {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            `[OmniCoder] onboardUser returned ${response.status} on attempt ${attempt + 1}`
          );
        }
      } catch (error) {
        const msg = error instanceof Error ? error.message : String(error);
        console.warn(`[OmniCoder] onboardUser attempt ${attempt + 1} failed (${msg})`);
      }
  ...
  async _doRefresh(accessToken: string, model = "unknown"): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[OmniCoder] loadCodeAssist returned ${response.status} — falling back to stored projectId`
        );
        return null;
      }
      ...
      if (!projectId) {
        console.warn(
          "[OmniCoder] loadCodeAssist returned no project — attempting managed project onboarding"
        );
      ...
      if (!projectId) {
        console.warn(
          "[OmniCoder] managed project onboarding failed — falling back to stored projectId"
        );
        return null;
      }
      ...
      const msg = error instanceof Error ? error.message : String(error);
      console.warn(`[OmniCoder] loadCodeAssist failed (${msg}) — falling back to stored projectId`);
      return null;
    }
  }
