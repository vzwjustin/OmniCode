...
export default function CodexToolCard({
  ...
  const getManualConfigs = () => {
    ...
    let configContent = `# OmniCoder Configuration for Codex CLI
model = "${selectedModel || CODEX_DEFAULT_MODELS[0]}"`;
...
# Utilize the built-in OpenAI provider pointed to OmniCoder
                ...
                <div className="flex items-center gap-2">
                  ...
                  {apiKeys.length > 0 ? (
                    ...
                    <span className="flex-1 text-xs text-text-muted px-2 py-1.5">
                      {cloudEnabled ? t("noApiKeysCreateOne") : t("defaultOmniCoderKey")}
                    </span>
                  )}
                </div>
