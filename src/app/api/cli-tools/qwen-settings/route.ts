...
const hasOmniRouteConfig = (settings: any) => {
  ...
  return openai.some((p: any) => {
    if (p.name?.includes("OmniCoder") || p.id === "omniroute") return true;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const anthropicEntry = {
      ...
      name: "Claude Sonnet 4.6 (OmniCoder)",
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    for (const type of providerTypes) {
      if (Array.isArray(existingConfig.modelProviders?.[type])) {
        existingConfig.modelProviders[type] = existingConfig.modelProviders[type].filter(
          (p: any) => !p.name?.includes("OmniCoder") && p.id !== "omniroute"
        );
    ...
    return NextResponse.json({
      success: true,
      message: "OmniCoder settings removed from Qwen Code",
    });
  } catch (error) {
